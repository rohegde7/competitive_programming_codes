...
no_tests = int(input()) #testcases

for i in range(no_tests):
    #print("Test case:", i+1)

    withstand_capacity_D, robot_prog = list(input().split())
    withstand_capacity_D = int(withstand_capacity_D)
    robot_prog = list(robot_prog)

    beam_strength = 1
    changes_req = 0
    total_damage = 0
    total_damage_test = 0
    withstand_capacity_test = withstand_capacity_D
    break_test = False

    #print(withstand_capacity_D, robot_prog)    #working

    for action in robot_prog:   #checking whether the default prog's total damage is < the withstand capacity
        if action is 'S':
            total_damage_test += beam_strength
        else:
            beam_strength *= 2

    total_damage = total_damage_test

    if total_damage < withstand_capacity_D:
        print("Case #%s:"%(i+1), changes_req)    #zero changes required
        continue

    while(total_damage > withstand_capacity_D):
        try:
            index_C = robot_prog.index('C') #finding the 1st occurance of 'C' in the prog
            #now we need to find the 1st occurance of 'S' after index_C in order to swap with it
            index_S = robot_prog[index_C+1:].index('S') + index_C+1
        except ValueError:
            print("Case #%s:"%(i+1), "IMPOSSIBLE")
            break_test = True
        ...
        #for situations where it is like: 'CCS'
        #we need to swap 2 'adjacent' commands
        if (index_S - index_C) != 1:
            index_C = index_S -1

        robot_prog[index_C], robot_prog[index_S] = robot_prog[index_S], robot_prog[index_C]
        changes_req += 1

        beam_strength = 1
        total_damage = 0

        for action in robot_prog:   #checking for total damage is < the withstand capacity
            if action is 'S':
    ...
    #print(index_C, index_S)

    if break_test == True:
        continue

    print("Case #%s:"%(i+1), changes_req)